...
(4) データ集合としての幾何的一致
をそれぞれ測るため，
Fréchet Inception Distance(FID)，Mean Intersection over Union (mIoU)，Layout Transportation Similarity（LTSim）~\cite{otani2024ltsim}，LTSim Maximum Mean Discrepancy（LTSim-MMD）~\cite{} の4指標を用いる．
特に LTSim は最適輸送に基づく柔軟な要素対応付けにより，
要素数やカテゴリの差異を含むレイアウト対も比較可能な指標として提案されている~\cite{otani2024ltsim}．
また，FID は分布比較に広く用いられる一方で，
知覚品質と必ずしも一致しない可能性も指摘されているため~\cite{otani2024ltsim}，
...
平均および共分散行列である．
特徴抽出器には LayoutDM~\cite{inoue2023layoutdm} に倣い
FIDNetV3 を用いる．
...
\paragraph{LTSim}
ページ単位での幾何的一致を評価するため，
最適輸送に基づく Layout Transportation Similarity（LTSim）~\cite{otani2024ltsim} を用いる．
LTSim は，
...
カテゴリ一致の有無を組み合わせたコストを用いる~\cite{otani2024ltsim}．
これにより，
...
layout-level の設定に従い，
$\sigma=1$ とする~\cite{otani2024ltsim}．
...
LTSim-MMD~\cite{otani2024ltsim} を用いる．
...
具体的には，
Swallow-13B，Sarashina-13B，
および DeepSeek-R1-Distill-Qwen-14B-Japanese を用いた．選定は，これらが高い日本語性能を有しており，日本語で記述された脚本の入力に対応できることと，オープンソースモデルであり自由にファインチューニングが可能であることを理由とした．
...
\begin{figure}[htbp]
   ...
   \caption{
      ...
      既存手法の生成例，
      Script2Layoutの生成例，
      が並び，Script2Layoutでは上段にScript条件なし，下段にScript条件ありの結果を示す．
   }
