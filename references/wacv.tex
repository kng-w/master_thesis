...
\usepackage[T1]{fontenc}     % ← 必須（lmodernとセット）         % Latin Modern で太字の見え方を安定
\usepackage{microtype}        % 任意だが推奨（表でも効く）
...
\usepackage{siunitx,makecell,booktabs,adjustbox}
\sisetup{
  mode = text,
  detect-weight=true,detect-inline-weight=math,
  detect-family=true,
  round-mode=places, round-precision=3,          % 小数は3桁表示（必要に応じて変更）
...
\input{sec/3_manga109scriptdataset}
